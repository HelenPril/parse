import scrapy
from ..proba_loaders import ProbaLoader
from gb_parse.spiders.xpath import HH_PAGE_XPATH, HH_VACANCY_XPATH


class ProbaSpider(scrapy.Spider):
    name = "proba"
    start_urls = ["https://hh.ru/search/vacancy?schedule=remote&L_profession_id=0&area=113/"]

    def _get_follow(self, response, selector, callback): #создается для того, чтобы выполнять похожую операцию в следующих двух ф-циях
        for url in response.xpath(selector):
            yield response.follow(url, callback=callback)

    def _get_follow_xpath(self, response, xpath, callback):
        for url in response.xpath(xpath):
            yield response.follow(url, callback=callback)

    def parse(self, response):
        callbacks = {"pagination": self.parse, "job": self.vacancy_parse}

        for key, xpath in HH_PAGE_XPATH.items():
            yield from self._get_follow_xpath(response, xpath, callbacks[key])

    def vacancy_parse(self, response):
        loader = ProbaLoader(response=response)
        loader.add_value("url", response.url)
        for key, xpath in HH_VACANCY_XPATH.items():
            loader.add_xpath(key, xpath)

        yield loader.load_item()

    def company_parse(self, response):
        pass